...
static pthread_rwlock_t readWrite = PTHREAD_RWLOCK_INITIALIZER;
...
void init(int numthreads) {
    if (numthreads != 1)
        printf("WARNING: This Trie is safe to use with multiple threads!!!  You have %d!!!\n", numthreads);
...
int search  (const char *string, size_t strlen, int32_t *ip4_address) {
    pthread_rwlock_rdlock(&readWrite);
